...
tape( 'the function returns `true` if provided `+infinity`', function test( t ) {
	t.strictEqual( isInfinitef( PINF ), true, 'returns expected value' );
	t.end();
});

tape( 'the function returns `true` if provided `-infinity`', function test( t ) {
	t.strictEqual( isInfinitef( NINF ), true, 'returns expected value' );
	t.end();
});
...
tape( 'the function returns `false` if not provided an infinite number', function test( t ) {
	...
	for ( i = 0; i < values.length; i++ ) {
		t.strictEqual( isInfinitef( values[i] ), false, 'returns expected value' );
	}
	t.end();
});
